    ...
    <div class="grid grid-cols-5 gap-4">
        ...
        <article class="flex flex-col items-start justify-between max-w-xl text-wrap">
            ...
            <div class="grid grid-rows-3 " style="max-width: 400px;"><!-- Adjust the max-width value according to your design -->
                ...
                <h3 class="row-span-2 mt-3 text-lg font-semibold text-gray-900 hover:bg-gray-100 hover:text-pink-900">
                    <a href="{{ route('posts.show', $post->id) }}" wire:key="$post->id">
                        <span class="inset-0 row-auto"></span>
                        {{$post->title}}
                    </a>
                </h3>
                <p class="row-span-2 mt-5 text-sm leading-6 text-gray-600 line-clamp-3 ">{!! implode(' ', array_slice(str_word_count($post->content, 1), 0, 50)) !!}</p>
            </div>
